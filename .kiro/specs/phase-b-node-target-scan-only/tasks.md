...
- [x] 1. NodeTargetDetector
  ...
  - [x] 1.1 Create `NodeTargetDetector.java` implementing `TargetDetector`
  ...
  - [x] 1.2 Write `NodeTargetDetectorTest.java`
  ...
  - [x] 1.3 Write `NodeDetectionProperties.java` (jqwik)
...
- [x] 2. NodeTarget skeleton + TargetRegistry registration
  ...
  - [x] 2.1 Create `NodeTarget.java` implementing `Target`
  ...
  - [x] 2.2 Register `NodeTarget` + `NodeTargetDetector` in `TargetRegistry.defaultRegistry()`
    - Add `TargetId.NODE → new NodeTarget()` to the targets map
    - Add `new NodeTargetDetector()` to the detectors list
  - [x] 2.3 Verify all existing JVM tests pass without modification

- [x] 3. TypeScript artifact generation
  ...
  - [x] 3.4 Create `TypeScriptManifestGenerator.java`
    - `generateWiringManifest(BearIr, Path, String)` → `<blockKey>.wiring.json`
  - [x] 3.5 Implement `NodeTarget.compile()` and `NodeTarget.generateWiringOnly()`
  ...
  - [x] 3.6 Implement `NodeTarget.parseWiringManifest()` and `NodeTarget.ownedGeneratedPrefixes()`
  - [x] 3.7 Write `TypeScriptArtifactGeneratorTest.java`
    - Each artifact type generated correctly; generated TypeScript parses without errors; user impl created once, not overwritten
  - [x] 3.8 Write `ArtifactGenerationProperties.java` (jqwik)
...
- [x] 4. Governed roots computation
  ...
  - [x] 4.1 Implement governed roots and file collection in `NodeTarget`
  ...
  - [x] 4.2 Write governed roots unit tests in `NodeTargetTest.java`
    - Single block, multi-block, with `_shared`, without `_shared`, test file exclusion, extension filtering
  - [x] 4.3 Write `GovernedRootsProperties.java` (jqwik)
...
- [x] 5. NodeImportSpecifierExtractor
  ...
  - [x] 5.1 Create `NodeImportSpecifierExtractor.java`
...
- [x] 6. NodeDynamicImportDetector
  ...
  - [x] 6.1 Create `NodeDynamicImportDetector.java`
...
- [x] 7. NodeImportBoundaryResolver
  ...
  - [x] 7.1 Create `NodeImportBoundaryResolver.java`
...
- [x] 8. NodeImportContainmentScanner
  ...
  - [x] 8.1 Create `NodeImportContainmentScanner.java`
  ...
  - [x] 8.2 Implement `NodeTarget.scanBoundaryBypass()` delegating to `NodeImportContainmentScanner.scan()`
  - [x] 8.3 Write `NodeImportContainmentScannerTest.java`
  ...
  - [x] 8.4 Write containment properties in `ImportContainmentProperties.java` (jqwik)
...
- [x] 9. Drift gate
  ...
  - [x] 9.1 Implement drift checking in `NodeTarget`
  ...
  - [x] 9.2 Write drift gate tests in `NodeTargetTest.java`
  ...
  - [x] 9.3 Write `DriftGateProperties.java` (jqwik)
...
- [x] 10. impl.allowedDeps unsupported guard
  ...
  - [x] 10.1 Implement `NodeTarget.blockDeclaresAllowedDeps()` — parse IR for `impl.allowedDeps` presence
  - [x] 10.2 Implement guard in check pipeline: if `blockDeclaresAllowedDeps()` and target=Node → fail, exit `64`, `CODE=UNSUPPORTED_TARGET`, IR file path, remediation message
  - [x] 10.3 Verify `pr-check` is unaffected by `impl.allowedDeps`
  - [x] 10.4 Write guard tests in `NodeTargetTest.java`
  ...
  - [x] 10.5 Write `AllowedDepsGuardProperties.java` or add to existing properties file (jqwik)
...
- [x] 11. Fixture projects + integration tests
  ...
  - [x] 11.1 Create fixture projects under `kernel/src/test/resources/fixtures/node/`
  ...
  - [x] 11.3 Run full JVM regression test suite; confirm zero failures

- [x] 12. TypeScript pretty printer
  ...
  - [x] 12.1 Implement consistent indentation and line break formatting in `TypeScriptArtifactGenerator`
  - [x] 12.2 Write pretty printer tests
    - Consistent formatting across invocations; output parseable by `tsc`; round-trip stability
  - [x] 12.3 Write pretty printer properties (jqwik)
